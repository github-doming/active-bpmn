<template>
  <div>
    <a-layout>
      ...
      <!-- 右边元素框 -->
      <a-layout-sider style="border: 1px solid #DCDFE6;background-color:#FAFAFA; height:80vh;" :width="silderWid">
        <a-form>
          <!-- 左拉右拉 UI -->
          <div class='icon' @click='changeIconShow'>
            <a-icon v-if='iconShow' type="double-left" />
            <a-icon v-else='iconShow' type="double-right" />
          </div>
    ...
    <a-layout-footer>
      <a-button @click="saveBpmn">保存</a-button>
      <a-button @click="downloadBpmn">下载</a-button>
