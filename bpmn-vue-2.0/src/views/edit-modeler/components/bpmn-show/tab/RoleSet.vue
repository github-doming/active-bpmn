			...
			<a-col :span="13" class='role-panel'>
				<table class="role-table">
					...
					<tr v-for="item in roleSetInfos" :key='item.id' :class="item.id === activeId ? 'selected':''"
						...
						<td>{{item.name}}</td>
						<td>
							<a-checkbox :checked="item.view">{{local.view}}</a-checkbox>
						</td>
						<td>
							<a-checkbox :checked="item.add">{{local.add}}</a-checkbox>
						</td>
						<td>
							<a-checkbox :checked="item.remove">{{local.remove}}</a-checkbox>
						</td>
					</tr>
					</tbody>
				</table>
